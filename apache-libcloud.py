...
class LibcloudInventory(object):
    ...
    def read_settings(self):
        ...
        # print(Driver)
        # print(self.conn)
        # print(self.key, self.secret,self.secure,self.host,self.path)
        # Cache related
    ...
    def get_node(self, node_id):
        ...
        '''
        return [node for node in self.conn.list_nodes() if node.name == node_id][0]
        # return [node for node in self.conn.list_nodes() if node.id == node_id][0]

    def parse_labels_from(self, note):
        result = {}
        if isinstance(note, basestring):
            kvs = note.split(";") #split input into lines
            for kv in kvs:
                if '=' in kv:
                    k, v = kv.split("=")
                    result[k] = v
